...
form.addEventListener("submit", (e)=>{
    ...
    if (valid) createMessageError(div);
    else createMessagSaccess(div);
...
tbody.addEventListener("dblclick", (e)=>{
    ...
    } else valueText = e.target.textContent;
    ...
    input.onkeydown = (even)=>{
        if (even.key === "Enter") {
            ...
            if (!input.value.trim() || input.value.trim().length < 4) e.target.textContent = oldValue;
            else e.target.textContent = input.value;
            input.remove();
        }
    };
    input.onblur = ()=>{
...
const createMessageError = (div)=>{
    ...
    if (inputName.value.trim().length < 4) titleElementName.textContent = "Name should be at least 4";
    if (!inputPosition.value.trim()) titleElementPosition.textContent = "Position value is invalid";
    if (inputSalary.value < 0) titleElementPosition.textContent = "salary must be greater than 0";
    if (inputAge.value < 18 || inputAge.value > 90) titleElement.textContent = "Age should be between 18 and 90 y.o.";
...
const checkValidValueError = ()=>{
    return inputAge.value < 18 || inputAge.value > 90 || inputSalary.value < 0 || inputName.value.trim().length < 4 || inputPosition.value.trim().length === 0;
};
